...
(function () {
  ...
  function safeActionError(error, fallback) {
    const message = String(error?.message || error || "").trim();
    return message && message.length <= 240 ? message : fallback;
  }
  async function toggleMount() { if (!invoke) return; const mounted = mountAction.dataset.mounted === "true"; mountAction.disabled = true; mountAction.textContent = mounted ? "Unmounting…" : "Mounting…"; try { const result = await invoke(mounted ? "unmount_stash" : "mount_stash"); applyMountStatus(result); setStatus(mounted ? "STASH was unmounted. Your files and cache were kept." : "STASH is mounted."); } catch (error) { mountAction.disabled = false; mountAction.textContent = mounted ? "Unmount STASH" : "Mount STASH"; setStatus(safeActionError(error, mounted ? "We couldn't unmount STASH. Try again." : "We couldn't mount STASH. Try again."), "error"); } }
