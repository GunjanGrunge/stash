...
(function () {
  ...
  async function loadMount() { if (!invoke) return; try { const result = await invoke("mount_status"); if (result?.mounted && result.letter) { mountStatus.textContent = `STASH (${result.letter}:)`; mountAction.textContent = "Mounted"; mountAction.disabled = true; } } catch (_) {} }
  async function mount() { if (!invoke) return; mountAction.disabled = true; mountAction.textContent = "Mounting…"; try { const result = await invoke("mount_stash"); const letter = result?.letter || result?.driveLetter; if (letter) { mountStatus.textContent = `STASH (${letter}:)`; mountAction.textContent = "Mounted"; setStatus("STASH is mounted and will remain available when this window closes."); } else throw new Error(); } catch (_) { mountAction.disabled = false; mountAction.textContent = "Mount STASH"; setStatus("We couldn't mount STASH. Try again.", "error"); } }
  ...
  mountAction?.addEventListener("click", mount);
